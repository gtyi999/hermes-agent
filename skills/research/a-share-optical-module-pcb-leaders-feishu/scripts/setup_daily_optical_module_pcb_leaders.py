#!/usr/bin/env python3
"""Install/update the daily A-share optical module/PCB leaders cron job."""
...
JOB_NAME = "Daily A-share Optical Module PCB Leaders to Feishu"
SKILL_NAME = "a-share-optical-module-pcb-leaders-feishu"
RUNTIME_FETCH_SCRIPT = "a_share_optical_module_pcb_leaders.py"
DEFAULT_SCHEDULE = "50 21 * * *"

CRON_PROMPT = """请根据 Script Output 中的 JSON，生成一条适合发送到飞书的中文 A 股光模块/PCB 概念龙头股行情日报。

要求：
1. 标题使用“光模块/PCB概念龙头股行情（YYYY-MM-DD）”，日期优先使用 trade_date。
2. 只输出 items 中最多 5 只股票；如果不足 5 只，按实际数量说明。
3. 在股票列表前输出“大盘指数”概览，必须包含 market_indices 中的上证指数和深证指数；每个指数包含最新点位、涨跌幅、涨跌额、成交额、成交量、数据时间。如果 market_indices 为空，说明指数数据暂不可用，并列出 market_index_errors 或 warnings 中的关键错误（如有）。
4. 每只股票包含：排名、代码、名称、命中概念、最新价、涨跌幅、涨跌额、成交额、成交量、换手率、主力净流入、总市值/流通市值。
5. 开头说明：行情日期、数据时间、覆盖概念板块、概念成分股去重数量、排序规则。
6. 如果 items 为空，说明当日暂无可用行情，并列出 concept_errors 或 warnings 中的关键错误（如有）。
7. 不要编造 Script Output 里没有的股票、题材原因或数据。
8. 末尾注明“仅供信息跟踪，不构成投资建议”。
9. 保持简洁，适合在飞书中阅读。
"""
...
def install_fetch_script(skill_dir: Path, hermes_home: Path) -> Path:
    source = skill_dir / "scripts" / "fetch_optical_module_pcb_leaders.py"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Install/update the daily optical module/PCB leaders cron job.")
    parser.add_argument("--schedule", default=DEFAULT_SCHEDULE, help="Cron schedule, default: 50 21 * * *")
