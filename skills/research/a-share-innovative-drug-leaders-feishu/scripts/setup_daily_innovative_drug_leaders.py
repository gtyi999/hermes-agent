#!/usr/bin/env python3
"""Install or update the daily innovative-drug leaders Feishu cron job."""
...
JOB_NAME = "Daily A-share Innovative Drug Leaders to Feishu"
SKILL_NAME = "a-share-innovative-drug-leaders-feishu"
RUNTIME_FETCH_SCRIPT = "a_share_innovative_drug_leaders.py"
DEFAULT_SCHEDULE = "18 23 * * *"

CRON_PROMPT = """请根据 Script Output 中的 JSON，生成一条适合发送到飞书的中文 A 股创新药概念龙头股行情日报。

要求：
1. 标题使用“创新药概念龙头股行情（YYYY-MM-DD）”，日期必须使用 trade_date；trade_date 为空时写“日期未知”，不得用抓取日期冒充交易日。
2. 只输出 items 中最多 5 只股票；如果不足 5 只，按实际数量说明。
3. 每只股票包含：排名、代码、名称、最新价、涨跌幅、涨跌额、开盘/最高/最低/昨收、成交额、成交量、换手率、主力净流入、总市值/流通市值。
4. 开头说明：最新交易日、数据时间、概念板块及代码、板块成分股数量、当日有效行情数量、排序规则。
5. 如果 fetched_at 的自然日与 trade_date 不同，明确说明当前为非交易日或数据源最新行情停留在 trade_date，不得称其为今日行情。
6. 如果 items 为空，明确说明暂无可用行情，并列出 errors 或 warnings 中的关键原因；数据源失败不能表述为“市场无股票”。
7. 不要编造 Script Output 中没有的股票、题材原因、交易数据或投资结论。
8. 末尾注明“数据来自东方财富公开行情，仅供信息跟踪，不构成投资建议”。
9. 保持简洁，适合在飞书中阅读；不要调用 send_message，Cron 会自动投递最终回复。
"""
...
def install_fetch_script(skill_dir: Path, hermes_home: Path) -> Path:
    source = skill_dir / "scripts" / "fetch_innovative_drug_leaders.py"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Install or update the daily innovative-drug leaders cron job.")
    parser.add_argument("--schedule", default=DEFAULT_SCHEDULE, help="Cron schedule; default: 18 23 * * *")
    parser.add_argument("--deliver", default="feishu", help="Delivery target, such as feishu:oc_xxx")
    ...
    if args.deliver.strip().lower() == "feishu" and not (
        ...
        warnings.append(
            "deliver=feishu needs FEISHU_HOME_CHANNEL in HERMES_HOME/.env; "
            "prefer --deliver 'feishu:<chat_id>'."
        )
