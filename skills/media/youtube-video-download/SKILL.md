...
Download YouTube videos with `yt-dlp`, preserving title metadata and merging
...
`ffmpeg` is needed when YouTube serves video and audio separately and `yt-dlp`
needs to merge them.
...
4. Add `--playlist` only when the user asks for playlist download.
5. If `yt-dlp` is missing, install it in the active environment and retry.
6. If merging fails, install or locate `ffmpeg`, then retry.
7. Report the output directory (`./downloads/youtube` by default) and any
...
- **`ffmpeg` missing**: install `ffmpeg`; without it, separate video/audio
  streams may not merge.
