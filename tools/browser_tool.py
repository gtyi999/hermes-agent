...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    try:
        ...
        def _take_screenshot_once(*, force_chrome: bool = False) -> Dict[str, Any]:
            screenshot_args = []
            if annotate:
                screenshot_args.append("--annotate")
            ...
            return _run_browser_command(
                ...
                "screenshot",
                screenshot_args,
                _engine_override="auto" if force_chrome else None,
            )
        ...
        if _lp_prerouted and screenshot_path.exists():
            ...
            result = _take_screenshot_once(force_chrome=_lp_prerouted)
        ...
        if _is_blank_browser_screenshot(screenshot_path):
            logger.warning(
                "browser_vision captured a blank screenshot; retrying once: %s",
                screenshot_path,
            )
            time.sleep(1)
            result = _take_screenshot_once(
                force_chrome=_lp_prerouted or engine == "lightpanda"
            )
            if not result.get("success"):
                error_response = {
                    "success": False,
                    "error": (
                        "Failed to retake blank screenshot: "
                        f"{result.get('error', 'Unknown error')}"
                    ),
                    "screenshot_blank": True,
                }
                return json.dumps(
                    _copy_fallback_warning(error_response, result),
                    ensure_ascii=False,
                )
            actual_screenshot_path = result.get("data", {}).get("path")
            if actual_screenshot_path:
        ...
        if _is_blank_browser_screenshot(screenshot_path):
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    "The browser session is likely still on a blank page or the page "
                    "did not finish rendering. Navigate to the target URL first, then "
...
def _is_blank_browser_screenshot(screenshot_path, tolerance=2) -> bool:
    ...
    try:
        ...
        with Image.open(screenshot_path) as image:
            ...
            return all(high - low <= tolerance for low, high in extrema)
    except ImportError:
        pass
    except Exception as exc:
        logger.debug(
            "Could not inspect screenshot for blank frame %s: %s",
            screenshot_path,
            exc,
        )
        return False
    ...
    return bool(png_result) if png_result is not None else False
...
def _is_blank_png_screenshot(screenshot_path, tolerance=2) -> Optional[bool]:
    ...
    if len(decompressed) < height * (row_len + 1):
        return None
    ...
    for _row in range(height):
        ...
        for idx in range(0, row_len, channels):
            ...
            if any(
                maxima[channel] - minima[channel] > tolerance
