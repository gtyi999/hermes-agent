...
def _create_app(adapter: APIServerAdapter) -> web.Application:
    ...
    app.router.add_post(
        ...
        adapter._handle_recruitment_ai_query,
    )
...
class TestRunEventCallback:
    ...
    def test_trace_safe_value_redacts_nested_credentials(self):
        secret = "sk-trace-secret-1234567890"
        value = _trace_safe_value({
            "api_key": secret,
            "nested": {
        ...
        assert secret not in value["nested"]["message"]

    @pytest.mark.asyncio
    async def test_tool_events_include_redacted_arguments_and_output(self, adapter):
        ...
        secret = "sk-tool-trace-abcdef1234567890"
        callback(
            "tool.completed",
            tool_name="example_tool",
            args={"api_key": secret, "query": "jobs"},
            result={"answer": "ok", "token": secret},
            duration=0.25,
        )

        event = await asyncio.wait_for(queue.get(), timeout=1.0)
        assert event["arguments"] == {"api_key": "[redacted]", "query": "jobs"}
        assert event["output"]["answer"] == "ok"
...
class TestRecruitmentSystemEndpoint:
    def test_route_table_includes_recruitment_query(self, adapter):
        routes = {(method, path) for method, path, _handler in adapter._http_route_table()}
        assert ("POST", "/api/recruitment-system/ai-query") in routes

    @pytest.mark.asyncio
    async def test_query_uses_recruitment_api_helper(self, adapter):
        app = _create_app(adapter)
        payload = {"user_question": "查询当前招聘岗位"}
        expected = {
            "success": True,
            "answer": "当前有 2 个招聘岗位。",
            "data": [{"id": 1}, {"id": 2}],
        ...
        with patch(
            "tools.recruitment_system_tool.query_recruitment_system_api",
        ...
        query.assert_called_once_with(payload)
