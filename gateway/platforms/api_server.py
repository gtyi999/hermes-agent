...
- POST /api/recruitment-system/ai-query — query RecruitmentSystem through its official API
...
def _trace_safe_value(value: Any, *, _depth: int = 0) -> Any:
    """Bound and recursively redact tool trace payloads before API delivery."""
    if _depth > 8:
        return "[max-depth]"
    ...
    if isinstance(value, str):
        ...
        if len(redacted) > MAX_TRACE_VALUE_LENGTH:
            return redacted[:MAX_TRACE_VALUE_LENGTH] + "\n[trace value truncated]"
        return redacted
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _http_route_table(self) -> List[tuple]:
        ...
        routes: List[tuple] = [
            ...
            ("POST", "/api/recruitment-system/ai-query", self._handle_recruitment_ai_query),
    ...
    async def _handle_recruitment_ai_query(self, request: "web.Request") -> "web.Response":
        """POST /api/recruitment-system/ai-query — read-only business query."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
