#!/usr/bin/env python3
"""Fetch current-trading-day optical module and PCB concept leader stocks.
...
USER_AGENT = "Mozilla/5.0 (compatible; HermesAgentOpticalPcbLeaders/1.0)"
...
DEFAULT_MARKET_INDICES = (
    ...
    {"name": "深证指数", "code": "399001", "secid": "0.399001"},
)
...
INDEX_FIELDS = "f12,f13,f14,f2,f3,f4,f5,f6,f7,f15,f16,f17,f18,f124"
...
def parse_concept_specs(raw_values: list[str] | None = None) -> list[dict[str, str]]:
    ...
    env_value = os.getenv("A_SHARE_OPTICAL_MODULE_PCB_LEADER_CONCEPTS", "").strip()
    if env_value:
...
def market_indices_url(host: str, index_specs: list[dict[str, str]]) -> str:
    params = {
        "fltt": "1",
        "invt": "2",
        "fields": INDEX_FIELDS,
        "secids": ",".join(spec["secid"] for spec in index_specs),
        "ut": QUOTE_TOKEN,
    }
    return f"{host}/api/qt/ulist.np/get?{urllib.parse.urlencode(params)}"
...
def normalize_market_index(item: dict[str, Any], requested_by_code: dict[str, dict[str, str]]) -> dict[str, Any]:
    code = str(item.get("f12", ""))
    quote_timestamp = item.get("f124")
    volume_hands = to_float(item.get("f5"))
    requested = requested_by_code.get(code, {})
    source_name = str(item.get("f14", ""))
    return {
        "code": code,
        "market": item.get("f13"),
        "market_label": market_label(item.get("f13"), code),
        "name": requested.get("name") or source_name,
...
def fetch_market_indices(index_specs: list[dict[str, str]] | None = None) -> list[dict[str, Any]]:
    specs = [dict(item) for item in (index_specs or DEFAULT_MARKET_INDICES)]
    requested_by_code = {spec["code"]: spec for spec in specs}
    errors = []

    for host in QUOTE_HOSTS:
        url = market_indices_url(host, specs)
        try:
            payload = fetch_json(url, timeout=10, retries=2)
            rows = ((payload.get("data") or {}).get("diff") or [])
            normalized_by_code = {
                item["code"]: item
                for item in (normalize_market_index(row, requested_by_code) for row in rows)
                if item.get("code") in requested_by_code
            }
            indices = [normalized_by_code[spec["code"]] for spec in specs if spec["code"] in normalized_by_code]
            if indices:
                return indices
            errors.append(f"{host}: empty requested index quote rows")
        except Exception as exc:  # noqa: BLE001
            errors.append(f"{host}: {exc}")

    raise RuntimeError("; ".join(errors) or "failed to fetch market indices")


def merge_stock_entry(
    ...
    current_ts = to_float(entry.get("quote_timestamp")) or 0
    row_ts = to_float(row.get("quote_timestamp")) or 0
    if row_ts >= current_ts:
        for key, value in row.items():
            if key not in {"concepts", "board_rows"}:
                entry[key] = value
...
def build_concept_index(concept_specs: list[dict[str, str]]) -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]], list[str]]:
    ...
    for spec in concept_specs:
        ...
        for row in constituents:
            merge_stock_entry(stock_index, row, board["name"])
...
def stock_sort_key(item: dict[str, Any]) -> tuple[float, float, float, float, float, str]:
    return (
        -sortable_number(item.get("total_market_value")),
        -sortable_number(item.get("turnover")),
        -sortable_number(item.get("concept_hit_count")),
...
def build_report(limit: int, concept_specs: list[dict[str, str]]) -> dict[str, Any]:
    ...
    market_index_errors: list[str] = []
    try:
        market_indices = fetch_market_indices()
    except Exception as exc:  # noqa: BLE001
        market_index_errors.append(str(exc))

    items: list[dict[str, Any]] = []
    for raw in stock_index.values():
    ...
    warnings = []
    if len(items) < limit:
        warnings.append(f"Only {len(items)} optical module/PCB concept stock quote(s) found.")
    if concept_errors:
        warnings.append("Some concept boards failed to load: " + "; ".join(concept_errors))
    if market_index_errors:
        warnings.append("Market indices failed to load: " + "; ".join(market_index_errors))

    return {
        "fetched_at": fetched_at,
        "source": RUNTIME_SOURCE,
        "market_indices": market_indices,
        "market_index_errors": market_index_errors,
        "concepts": boards,
        ...
        "schedule_note": "The report is scheduled for 21:50; trade_date is the latest quote date returned by Eastmoney.",
        "concept_constituent_count": len(stock_index),
        "ranked_count": len(items),
        "limit": limit,
        "ranking_rule": "total_market_value desc, turnover desc, concept_hit_count desc, main_net_inflow desc, change_pct desc",
        "items": top,
        "warnings": warnings,
        "source_urls": {
            "board_search": "https://searchapi.eastmoney.com/api/suggest/get",
            "market_indices": "https://push2.eastmoney.com/api/qt/ulist.np/get",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Fetch A-share optical module and PCB concept leader stock quotes.")
