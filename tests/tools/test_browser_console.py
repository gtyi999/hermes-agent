...
class TestBrowserVisionConfig:
    ...
    def test_browser_vision_native_fast_path_caps_history_embed(self, tmp_path):
        ...
        shots_dir.mkdir()
        screenshot = shots_dir / "shot.png"
        # Taller than the long-edge cap so the resize path must fire. Include
        # contrasting content so the branch's blank-frame guard does not
        # intentionally reject this synthetic screenshot as a single-color
        # capture failure before the history-embed path is exercised.
        image = Image.new("RGB", (400, _EMBED_MAX_DIMENSION + 500), (0, 100, 0))
        image.paste((240, 240, 240), (40, 40, 180, 180))
