#!/usr/bin/env python3
"""Install/update the daily A-share solid-state battery leaders cron job."""
...
JOB_NAME = "Daily A-share Solid-state Battery Leaders to Feishu"
SKILL_NAME = "a-share-solid-state-battery-leaders-feishu"
RUNTIME_FETCH_SCRIPT = "a_share_solid_state_battery_leaders.py"
DEFAULT_SCHEDULE = "50 22 * * *"

CRON_PROMPT = """请根据 Script Output 中的 JSON，生成一条适合发送到飞书的中文 A 股固态电池概念龙头股行情日报。

要求：
1. 标题使用“固态电池概念龙头股行情（YYYY-MM-DD）”，日期优先使用 trade_date。
2. 只输出 items 中最多 5 只股票；如果不足 5 只，按实际数量说明。
3. 每只股票包含：排名、代码、名称、最新价、涨跌幅、涨跌额、成交额、成交量、换手率、主力净流入、总市值/流通市值。
4. 开头说明：行情日期、数据时间、概念板块、成分股数量、排序规则。
5. 如果 items 为空，说明当日暂无可用行情，并列出 errors 或 warnings 中的关键错误（如有）。
...
def install_fetch_script(skill_dir: Path, hermes_home: Path) -> Path:
    source = skill_dir / "scripts" / "fetch_solid_state_battery_leaders.py"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Install/update the daily solid-state battery leaders cron job.")
    parser.add_argument("--schedule", default=DEFAULT_SCHEDULE, help="Cron schedule, default: 50 22 * * *")
